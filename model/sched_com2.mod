...
//can be precomputed: a matching value 
dexpr int networkMatch[f in Flows][n in Networks] =
		((//jitter
			importance[IMP_JIT, f]*
			(network_jit[n]-jitter[f])*(network_jit[n]-jitter[f])*
			( (network_jit[n]-jitter[f]) >=0)
		)+(//latency
			importance[IMP_LCY, f]*		
			(network_lcy[n]-latency[f])*(network_lcy[n]-latency[f])*	//latency violation square
			( (network_lcy[n]-latency[f]) >=0)
		)+(//monetary cost
			network_cost[n]*costImp
		)-(//non_allocated
...
*/
dexpr int matching_cost[f in Flows] = sum(t in Time, n in Networks)(allocatedChunks[f,t,n]*networkMatch[f,n]+importance[IMP_UNSCHED, f]*nChunks[f]);
dexpr int cost_violation = sum(f in Flows)((st_vio[f]+dl_vio[f]+vioThroughput[f]+matching_cost[f])*userImportance[f]);

dexpr int cost_total = cost_violation+//cost_ch*costImp+
						cost_switch;
...
subject to{
	...
	//allocate resources
	forall (f in Flows)( 
		sum (t in Time, n in Networks)allocatedChunks[f,t,n] //+ non_allocated[f] == 
			<= nChunks[f] 
	);
	...
	/********************************************************************/  
	//punish channel switches (ping pong avoidance)
