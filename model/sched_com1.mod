...
dexpr int vioMatch[f in Flows] =
		sum(n in Networks, t in Time) 
			( match[f,n]*allocatedChunks[f,t,n]);
...
dexpr int cost_violation = sum(f in Flows)((st_vio[f]+dl_vio[f]+non_allo_vio[f]+vioThroughput[f]+vioMatch[f]+nChunks[f]*userImportance[f]*importance[IMP_UNSCHED,f]/*+vioLcy[f]+vioJit[f]*/)*userImportance[f]);
...
subject to{
	...
	forall(f in Flows, t0 in Time, t1 in Time)(
		(t0>=prefStartTime[f])*(t1<=deadline[f])*(t1-t0==tpMaxWindow[f]-1)<=(sum(n in Networks, t in t0..t1)allocatedChunks[f,t,n]//cummulatedChunks[f, t1]-cummulatedChunks[f, t0]
	...
	forall(f in Flows, t0 in Time, t1 in Time)(
		(t0>=prefStartTime[f])*(t1<=deadline[f])*(t1-t0==tpMinWindow[f]-1)<=(//cummulatedChunks[f, t1]-cummulatedChunks[f, t0]
		sum(n in Networks, t in t0..t1)allocatedChunks[f,t,n]>=tpMinChunks[f]-vioTpMin[f,t1] )
