...
//Schedule
dvar int+ allocatedChunks[Requests, Time, Channels];	//allocate
...
dexpr int st_vio[r in Requests] = 
	sum(c in Channels, t in Time)(
		...
		allocatedChunks[r, t, c]			//linear weight on BW of violation
	);
//deadline violation
dexpr int	dl_vio[r in Requests] = 
	sum(c in Channels, t in Time)(
		...
		allocatedChunks[r, t, c]				//linear factor of punishment concerning bandwidth during violation
	);
...
dvar int closed_gaps[Requests, Time, Channels] in 0..1;
...
dexpr int cost_ch = sum(r in Requests, t in Time, c in Channels)(allocatedChunks[r, t, c]*channel_cost[c]);
...
subject to{
	...
	forall (r in Requests)( 
		sum (t in Time, c in Channels)allocatedChunks[r,t,c] + non_allocated[r] == nChunks[r] 
	);
		
	//do not overuse available resources
	forall (t in Time, c in Channels)(
	  	sum (r in Requests)allocatedChunks[r, t, c] <= availBW[c, t]
	);
	
	//do not allow parallel channel use for single request
	forall (r in Requests, t in Time, c1 in Channels, c2 in Channels)(
		(c1!=c2)	<=	((allocatedChunks[r,t,c1]==0) + (allocatedChunks[r,t,c2]==0))
	);
	
	/********************************************************************/  
	//puhish channel switches (ping pong avoidance)
	forall (r in Requests, t in Time)(
		sum (c in Channels)(closed_gaps[r, t, c])==1		//no gaps: in each timeslot sum is one; so channel selection takes place even if no traffic is scheduled
	);
	forall (r in Requests, t in Time, c in Channels)(
		closed_gaps[r, t, c]>=(allocatedChunks[r, t, c]!=0) //force equality of scheduled chunks to channel selection
	);
	...
	forall (t in Time, it in InterfaceTypes)(
		sum(c in Channels)(		//sum channels of type it	
			(( sum (r in Requests) allocatedChunks[r,t,c]) >= 1)  //channel is used and
	...
	forall(r in Requests)( //first timeslot is 0+first
		cummulatedChunks[r, 1] == sum(c in Channels)allocatedChunks[r, 1, c]
	);
