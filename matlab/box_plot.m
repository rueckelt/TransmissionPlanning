...
%       are returned in H.SCATTER. Data that are outliers will not be plotted.
%   'scatterMarker'       : '+' | 'o' | '*' | '.' | {'x'} |
...
% =========================================================================
% Last changed:     $Date: 2015-10-01 11:20:36 +0100 (Thu, 01 Oct 2015) $
% Last committed:   $Revision: 430 $
...
% =========================================================================
    ...
    for n = 1:prod(outdims)
        ...
        gOffset = get_offset(gRange,outdims(3:end),subidx(3:end));
        ...
        if options.notch % if notch requested
            ...
            p = patch([x(subidx{2})-halfboxwidth x(subidx{2})-halfboxwidth x(subidx{2})-halfboxwidth+notchdepth x(subidx{2})-halfboxwidth x(subidx{2})-halfboxwidth ...
                ...
                Z.Q3(subidx{:}) Z.notch_u(subidx{:}) Z.median(subidx{:}) Z.notch_l(subidx{:}) Z.Q1(subidx{:})],'w');
        else
            % box
            p = patch([x(subidx{2})-halfboxwidth x(subidx{2})-halfboxwidth x(subidx{2})+halfboxwidth x(subidx{2})+halfboxwidth]+gOffset,...
                [Z.Q1(subidx{:}) Z.Q3(subidx{:}) Z.Q3(subidx{:}) Z.Q1(subidx{:})],'w');
        end
        ...
        case 'hierarchy'
            ...
            if ~isempty(options.groupLabels)
                ...
                assert(isequal(outdims(3:end),cellfun(@length,options.groupLabels)),...
                    ...
                    int2str(outdims(3:end)) ' != ' int2str(size(cellfun(@length,options.groupLabels)))])
            end
...
function offset = get_offset(range,dims,subidx)
%GET_OFFSET calculate offset for hierarchical data

    width = (range*prod(dims))/(prod(dims)-1); % end correction
    offset = -range/2; % startin value
    for c = 1:length(dims) % work through dims
        width = width/(dims(c)); % decrease available space for higher dimensions
        offset = offset + (subidx{c}-1)*width; % add space to offset
    end
